...
void merger(multiset< rect , custom_compare> & state
    ...
    bool ok=1;
    //now lets only loop from one element before the inserted position?
    ll limit = itr->right;//after this we cannot have any effect..
    
    for(auto i = state.begin();
        i!=itr ; i++){
        stak.push_back(*i);//push them without any comparison!
    }
    
    while(ok){
        if(itr == state.end())break;
        ...
        if(((itr)->left)>limit ){
            //no use doing merge ahead of this!
...
// ll perimeter(const vector<rect>&hd){
...
// ll perimeter(multiset< rect , custom_compare > & state){
